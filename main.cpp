...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	result = LoadFromWICFile(
		L"Resources/texture.jpg",	//「Resources」フォルダの「texture.jpg」
		WIC_FLAGS_NONE,
		&metadata, scratchImg);
	...
	srvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;		//2Dテクスチャ
	...
	while (true)
	{
		...
		if (key[DIK_SPACE])
		{
			clearColor[0] = { 0.0f };	//黒っぽい色
			clearColor[1] = { 0.0f };	//黒っぽい色
			clearColor[2] = { 0.0f };	//黒っぽい色
			clearColor[3] = { 0.0f };	//黒っぽい色
		}

		if (!key[DIK_SPACE])
		{

			clearColor[0] = { 0.1f };	//青っぽい色
			clearColor[1] = { 0.25f };	//青っぽい色
			clearColor[2] = { 0.5f };	//青っぽい色
			clearColor[3] = { 0.0f };	//青っぽい色
		}
